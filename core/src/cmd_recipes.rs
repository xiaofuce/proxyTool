...
//! 落盘即加密。加密原语在 `crypto.rs` (AES-256-GCM, 与 secrets 共用),
//! 安全分级同彼处说明 (防顺手查看级, 非对抗性)。
//!
...
const ENC_FILE: &str = "cmd_recipes.enc";
const KEY_FILE: &str = "cmd_recipes.key";
...
mod tests {
    ...
    fn enc_path(d: &std::path::PathBuf) -> std::path::PathBuf {
        d.join(ENC_FILE)
    }
    fn key_path(d: &std::path::PathBuf) -> std::path::PathBuf {
        d.join(KEY_FILE)
    }
    ...
    fn resave_keeps_key_stable() {
        ...
        let key1 = std::fs::read(key_path(&d)).unwrap();
        let mut s = sample();
        s.recipes.clear();
        save_cmd_store(&d, &s).unwrap();
        let key2 = std::fs::read(key_path(&d)).unwrap();
    ...
    fn corrupt_or_keyless_is_empty() {
        ...
        save_cmd_store(&d, &sample()).unwrap();
        std::fs::remove_file(key_path(&d)).unwrap();
        assert!(load_cmd_store(&d).recipes.is_empty());
