...
一个小工具，一键建立「本地电脑 ⇄ 远程服务器」的 SSH 隧道。所有操作在本地 PC 完成，只需远程服务器账密（密码以 AES-256-GCM **密文**保存在本机，重启免重输）。
...
| **服务器** | — | 保存常用服务器配置（名称/地址/端口/用户名/私钥路径），一键填入各隧道表单；密码单独加密存储（secrets.enc），可随时清除 |
